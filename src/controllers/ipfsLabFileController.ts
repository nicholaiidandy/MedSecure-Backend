...
export const uploadLabFileToIPFS = async (req: Request, res: Response) => {
  try {
    ...
    if (pinataJwt) {
      ...
      if (isInfuraApi && (!ipfsProjectId || !ipfsProjectSecret)) {
        return res.status(500).json({
          success: false,
          message:
            'IPFS is not configured. Set PINATA_JWT or set INFURA_PROJECT_ID and INFURA_PROJECT_SECRET in backend environment.',
        });
      }
      ...
      try {
        ...
        if (isInfuraAuthError) {
          return res.status(502).json({
            success: false,
            message:
              'Infura IPFS authentication failed. Set valid INFURA_PROJECT_ID/INFURA_PROJECT_SECRET or use PINATA_JWT.',
          });
        }

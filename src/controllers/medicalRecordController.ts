...
// @access  Private
export const getRecordsByPatient = async (req: AuthRequest, res: Response) => {
  try {
...
// @access  Private
export const verifyRecord = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        recordDetails: {
          patient: (record.patient as any)?.name || 'Unknown',
          diagnosis: record.diagnosis,
          date: record.date ? record.date.toISOString().slice(0, 10) : '',
          patientId: (record.patient as any)?.patientId || '',
        },
      },
    });
  } catch (error) {

...
export const getProfile = async (req, res) => {
    try {
        ...
        const user = await withMetrics(User.findById(userId).select('-password'), 'User', 'findById_profile');
        if (!user) {
...
export const updatePassword = async (req, res) => {
    ...
    try {
        ...
        const user = await withMetrics(User.findById(userId).select('+password'), 'User', 'findById_change_password_profile');
        if (!user) {
            return res.status(404).json({
                success: false,
                message: 'User not found'
...
export const toggle2FA = async (req, res) => {
    ...
    try {
        ...
        const user = await withMetrics(User.findByIdAndUpdate(userId, { twoFactorEnabled: enabled }, { new: true, runValidators: true }).select('-password'), 'User', 'findByIdAndUpdate_2fa');
        if (!user) {
            return res.status(404).json({
                success: false,
                message: 'User not found'
